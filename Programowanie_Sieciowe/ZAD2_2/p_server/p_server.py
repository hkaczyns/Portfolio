...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Delay between receiving nodes (seconds)",
    )

    return parser.parse_args()


def unpack_nodes(conn: socket.socket, addr: tuple, delay: float) -> None:
    print(f"Connect from {addr}")
    print(f"Slow receiver - delay is {delay} seconds between reads")
