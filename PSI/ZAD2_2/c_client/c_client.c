...
#define Usage() { errx(0, "Usage: %s [address-or-ip] [port] [number-of-nodes] [buffer-size]\n", argv[0]); }
...
void send_list(int sock, Node *head) {
    ...
    while (current != NULL) {
        ...
        double elapsed_time = end_ms - start_ms;

        if (elapsed_time > max_time) max_time = elapsed_time;
        if (elapsed_time < min_time) min_time = elapsed_time;
        // "Slow write" defined as taking more than 0.1 millisecond
        if (elapsed_time > 0.1) slow_writes++;
        total_time += elapsed_time;
        
        count++;
        printf("I've just sent a node %d: val16=%d, val32=%d, text=\"%s\"\n", count, current->val16, current->val32, current->text);
        printf("Time taken to send node %d: %lf milliseconds\n", count+1, elapsed_time);
        
        current = current->next;
    }
    
    printf("\nTotal nodes sent: %d\n", count);
    printf("Max time taken to send a node: %lf milliseconds\n", max_time);
    printf("Min time taken to send a node: %lf milliseconds\n", min_time);
    printf("Average time per node: %lf milliseconds\n", count > 0 ? total_time / count : 0);
    printf("Number of slow writes: %d\n", slow_writes);
}
...
int main(int argc, char *argv[]) {
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_SNDBUF, &buffer_size, sizeof(buffer_size)) == -1) {
        bailout("setsockopt SO_SNDBUF");
    }
