...
export const EnrollmentModal = ({ classGroup, isOpen, onClose, onConfirm, isLoading }: EnrollmentModalProps) => {
        ...
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.modalContent} onClick={(e: React.MouseEvent) => e.stopPropagation()}>
                <div className={styles.modalHeader}>
                    <h2 className={styles.modalTitle}>Potwierdzenie zapisu</h2>
                    <button className={styles.closeButton} onClick={onClose} aria-label="Zamknij">
                        <X size={24} />
                    </button>
                </div>
                <div className={styles.modalBody}>
                    <p className={styles.confirmationText}>Czy na pewno chcesz zapisać się na następujące zajęcia?</p>
                ...
                <div className={styles.modalFooter}>
                    ...
                    <button className={styles.confirmButton} onClick={onConfirm} disabled={isLoading}>
                        {isLoading ? "Zapisywanie..." : "Potwierdzam zapis"}
                    </button>
                </div>
            </div>
        </div>
