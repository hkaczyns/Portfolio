...
async def test_student_can_view_calendar(
    ...
    enrolled_group = await create_class_group(
        client, deps, str(instructor.id), "Salsa - Enrolled"
    )
    other_group = await create_class_group(
        client, deps, str(instructor.id), "Salsa - Other"
    )

    for session_date in ["2024-01-08", "2024-01-15"]:
        response = await client.post(
            f"{settings.API_V1_STR}/admin/schedule/class-sessions",
            json={
                "classGroupId": enrolled_group["id"],
                "date": session_date,
                "startTime": "18:00",
                "endTime": "19:30",
                "roomId": deps["room_id"],
    ...
    await login(client, "calendar_student@example.com", "Student_Password1")

    calendar_response = await client.get(
        f"{settings.API_V1_STR}/me/calendar",
    ...
    assert all(
        entry["classGroupId"] == enrolled_group["id"] for entry in entries
    )
